...
(defstruct (mem (:type vector)) 
  (index-file-path #'(lambda (filename) (format nil "/home/vb/repository/portfolio/crawler/data/index/~A" filename)))
  (dict-size 0)
  (num-rec 0)
  (total (* 20 1024 1024))
  (block-size (* 64 1024))
...
(defun index-path->file (index-name file)
  (format nil "/home/vb/repository/portfolio/crawler/data/index/~A/~A" index-name file))

(defun encode-gaps->bytes (gaps encoding-scheme)
  (mappend #'(lambda (gap) (encode-triple-gap gap encoding-scheme)) gaps))

(defun create-lexicon (mem)
  (write-hashtable (mempath mem "terms") (mem-dict mem)))

(defun mempath (mem filename)
  (funcall (mem-index-file-path mem) filename))

(defun write-run-table (mem)
  (write-hashtable (mempath mem "runs") (mem-run-table mem)))
...
(defun flush-in-memory-index->disk (mem encoding-scheme padd-byte)
  (with-open-file (temp (mempath mem "temp")
...
(defun multiway-in-place-sort-inverter (index-name 
					&key 
					(restart-p t)
					(mem (make-mem))
					(filename nil)
					...
					(encoding-scheme '(bits encode-unary encode-delta encode-unary))
					(mem-indexer #'in-memory-index)
					(padd-byte 0))
      (let ((docid 0) (doc ""))
	(setf (mem-index-file-path mem) #'(lambda (filename) (index-path->file index-name filename)))
	(ensure-directories-exist (mempath mem ""))
	(when (probe-file (mempath mem "temp")) (delete-file (mempath mem "temp")))
	(print 'init)
	(loop
	   (setf doc (funcall doc-reader))
	   (when (or (not doc) (and (stringp doc) (string= doc "")))
	     (flush-in-memory-index->disk mem encoding-scheme padd-byte)
	     (memory-status mem)
	     (return 'done))
	   (progn
	...
	(write-run-table mem)
	(print 'index-details)
	(describe-index index-name mem parse-document encoding-scheme padd-byte filename)
...
(defun describe-index (index-name mem parse-document encoding-scheme padd-byte filename)
  (with-open-file (des (mempath mem "describe") :direction :output :if-exists :supersede :if-does-not-exist :create)
...
(defun merge-runs (mem &key (encoding-fn #'encode-variable-bytes))
  (let ((heap (create-heap #'triple-sorter))
	...
	(when-bind (it (next-triple (1+ run) mem))
	  (heap-insert heap it)))
      (with-open-file (index (mempath mem "index") 
			     :direction :output 
	...
	(when records (write-record index)))
      (print 'done)
      (let ((terms (read-hashtable (mempath mem "terms"))))
	(maphash #'(lambda (term termid)
		     (setf (gethash term terms) (gethash termid lex)))
		 terms)
	(write-hashtable (mempath mem "lexicon") terms)))))

(defun merge-runs-old (mem &key (encoding-fn #'encode-variable-bytes))
  (let ((heap (create-heap #'triple-sorter))
	(lex (make-hash-table :size 1500000))
	(records '())
	(prv 0)
	...
	(total 0))
    (labels ((write-record (index)
	       (setf records (map-adj #'(lambda (x y) (cons (- (car y) (car x)) (cdr y)))
				      #'first
				      #'second
				      #'cdr
				      (cons init
					    (nreverse records))))
	       (let* ((r (mapcan #'(lambda (gap) (append (funcall encoding-fn (car gap))
							 (funcall encoding-fn (cdr gap))))
				 records))
		      (len (length r))) 
		 (incf total len)
		 (write-sequence r index)
		 (setf (gethash (triple-termid last) lex)
		       (cons prv (incf prv len)))
		 (setf records nil)
		 (setf last (make-triple)))))
      (dotimes (run (mem-run mem))
	(awhen (next-triple (1+ run) mem)
	  (heap-insert heap it)))
      (with-open-file (index (mempath mem "index") 
			     :direction :output 
			     :element-type 'unsigned-byte 
...
(defun read-next-byte (run-num mem)
  (let ((run (gethash run-num (mem-run-table mem))))
	...
	(if (nothing-to-read? run mem)
	    ...
	    (let ((seq (make-array (mem-block-size mem) :element-type 'unsigned-byte :fill-pointer t)))
	      (with-open-file (temp (mempath mem "temp") :direction :input :element-type 'unsigned-byte)
...
(defun searcher (index-name)
  (let ((lexicon (read-hashtable (index-path->file index-name "lexicon"))))
    #'(lambda (dispatch)
	(case dispatch
	  (:lexicon #'(lambda (term)
		    ...
		    (map-adj #'(lambda (x y) (declare (ignore x)) (cons (incf docid (car y)) (cdr y)))
			     ...
			     (append '(0 0)
				     (read-all-variable-bytes 
				      (read-bytes 
				       (index-path->file index-name "index")
				       (car it) 
				       (cdr it))))))
		  :test #'= :key #'car))))))))
